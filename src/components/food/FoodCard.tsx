  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-lg border border-charcoal-700 bg-charcoal-800">
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        {categoryName && (
          <span className="text-xs font-medium uppercase tracking-wide text-primary-300">{categoryName}</span>
        )}
        ...
        {hasDiscount ? (
          <p className="flex items-center gap-2">
            ...
            <span className="font-medium text-primary-200">{formatPrice(discountedPrice)}</span>
            <span className="rounded-full bg-primary-400/20 px-2 py-0.5 text-xs font-medium text-primary-200">
              -{food.discount_percentage}%
            </span>
          </p>
        ) : (
          <p className="font-medium text-primary-200">{formatPrice(food.price)}</p>
        )}
      </div>
    </article>
  )
